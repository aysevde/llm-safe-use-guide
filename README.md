...
> **Note:** `set` is temporary and only lasts for that terminal session. For a permanent variable, use `setx OPENAI_API_KEY "sk-abc123secretkey456"` instead.
...
- **Start with free tiers** since most services offer them for students
- **Be careful with loops** because one bad `while` loop can make hundreds of API calls in seconds and drain your credits
...
A `.gitignore` file tells Git which files to ignore. These files will never be committed or pushed to GitHub.
...
- **Database connection strings** because they contain credentials
- **Personal data** such as names, emails, phone numbers, student IDs
...
- **Set constraints.** For example: "do not use external libraries" or "keep it under 20 lines."
...
- **Run it in a safe environment** such as a Jupyter notebook or a test script.
